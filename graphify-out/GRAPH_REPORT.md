...
## Corpus Check
- 49 files · ~44,067 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 693 nodes · 1939 edges · 22 communities (18 shown, 4 thin omitted)
- Extraction: 85% EXTRACTED · 15% INFERRED · 0% AMBIGUOUS · INFERRED: 285 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f34dd0aa`
...
1. `New()` - 40 edges
2. `runCLI()` - 36 edges
3. `withConfigDir()` - 31 edges
4. `baseTime()` - 27 edges
5. `RenderGalaxyDetail()` - 24 edges
6. `RenderQwenDetail()` - 24 edges
7. `Config` - 22 edges
8. `QwenRepo` - 21 edges
9. `Load()` - 19 edges
10. `Colorizer` - 19 edges
...
Cohesion: 0.08
Nodes (79): QwenResult, Builder, PlanDisplayName(), galaxyResult(), T, TestGalaxyOverviewLowBalanceRed(), TestGalaxySpanShort(), TestGalaxyStatusColor() (+71 more)
...
### Community 2 - "main.go"
Cohesion: 0.09
Nodes (68): checkPermissions(), DefaultPath(), Load(), NewID(), T, TestDefaultPathHome(), TestDefaultPathXDG(), TestLoadCorruptJSON() (+60 more)

### Community 3 - "QwenRepo"
Cohesion: 0.11
Nodes (22): QwenCLIDefaultLoginCmd(), cookieValue(), defaultClient(), doGet(), Client, Duration, Time, joinErrors() (+14 more)
...
Cohesion: 0.07
Nodes (57): AggregateGalaxyCost(), GalaxyDeadlineUnix(), galaxyMissingField(), GalaxyRFC3339(), GalaxySign(), GalaxyStatusActive(), GalaxyStatusText(), GalaxyStringToSign() (+49 more)

### Community 8 - "runCLI"
Cohesion: 0.17
Nodes (39): galaxyEnv(), T, TestGalaxyAddAndListMasksSecrets(), TestGalaxyAddBadTypeExit2(), TestGalaxyAddFromEnvVars(), TestGalaxyAddMissingSecretNonTTY(), TestGalaxyFlagAfterName(), TestGalaxyNoRefreshListsAccount() (+31 more)
...
### Community 22 - "New"
Cohesion: 0.07
Nodes (31): AccountResult, App, DeepSeekResult, GalaxyResult, Result, Config, errMsg(), Time (+23 more)

## Knowledge Gaps
- **75 isolated node(s):** `github.com/xieguiawu/llm-api-check`, `build-dist.sh script`, `最后一次完成的工作（2026-08-30 15:0x）`, `最后一次完成的工作（2026-08-30 14:35）`, `待办：Android 对等实现` (+70 more)
...
- **Why does `New()` connect `New` to `parsers_test.go`, `main.go`, `QwenRepo`, `qwen_cli_test.go`, `NewWithRepos`, `parsers/galaxy_test.go`?**
